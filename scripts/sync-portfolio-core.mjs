...
// [!todo] callouts are editorial notes-to-self and are stripped here before
// publishing. Any other `[!type]` callout (note/tip/warning/etc.) is left
// untouched on purpose — those are rendered client-side by remarkCallout, not
// filtered out.
...
export function resolveContentRoot() {
  return path.resolve(process.env.PORTFOLIO_CONTENT_ROOT || defaultContentRoot);
}
